...
fastfact2 = function(Y, prop = 1, epsilon = 1e-3, nrun, burn, thin = 1, 
  ...
  tauh = 1 / exp(cumsum(log(theta)))                                       # global shrinkage coefficients
  ...
  for(i in 1:nrun) {
    ...
    mat = psijh * Lambda^2
    ad = ad1 + 0.5*p*k
    bd = bd1 * theta[2] + 0.5 * theta[1] * sum(tauh*colSums(mat))
    theta[1] = 1 / rgamma(1,ad,bd)           
    tauh = 1 / exp(cumsum(log(theta)))
    
    
    for(h in 2:(k-1)) {
      ad = ad2 + 0.5*p*(k-h+1)
      bd = bd2 * (theta[h-1] + theta[h+1]) + 0.5 * theta[h] * sum(tauh[h:k]*colSums(mat[,h:k, drop = F]))
      theta[h] = 1 / rgamma(1,ad,bd)
      tauh = 1 / exp(cumsum(log(theta)))
    }
    
    ad = ad2 + 0.5*p
    bd = bd2 * (theta[k-1]) + 0.5 * theta[k] * sum(tauh[k]*colSums(mat[,k, drop = F]))
    theta[k] = 1 / rgamma(1,ad,bd)
    tauh = 1 / exp(cumsum(log(theta)))
